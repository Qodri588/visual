...
export function createDefaultProject(): ProjectSettings {
  return {
    ...
    effectAmount: .45 + Math.random() * 1.25,
    effectSpeed: .25 + Math.random() * 2.5,
    effectSize: 1 + Math.random() * 2,
    effectDirection: 'random',
    effectOrigin: 'random',
    effectOpacity: 1,
    effectGlossy: Math.random(),
    effectSizeMode: 'random',
    effectBeatReactive: true,
